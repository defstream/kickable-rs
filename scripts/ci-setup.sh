apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends zip qemu-system binfmt-support qemu-user-static x86_64-w64-mingw32-gcc gcc-x86-64-linux-gnu mingw-w64 iptables
